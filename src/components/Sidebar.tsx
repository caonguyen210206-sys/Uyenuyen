...
import { Home, LibraryBig, ListTodo, PenTool, CalendarDays, Settings as SettingsIcon, Sprout, Link2, X, ShieldAlert, FolderKanban } from 'lucide-react';
...
export default function Sidebar({ currentView, setCurrentView, isOpen = false, onClose }: SidebarProps) {
  const menuItems = [
    ...
    { id: 'reading-projects', label: 'Reading Projects', icon: FolderKanban },
  ...
  return (
    <>
      {isOpen && (
        <button aria-label="Đóng menu" onClick={onClose} className="fixed inset-0 z-40 bg-black/35 backdrop-blur-[2px] lg:hidden" />
      )}
      ...
      <aside className={cn(
        ...
        <div className="flex items-start justify-between gap-3 mb-8 px-1 mt-2 lg:mt-4 lg:block lg:mb-10 lg:px-2">
          ...
          <button aria-label="Đóng menu" onClick={onClose} className="lg:hidden p-2 rounded-xl bg-white/70 text-[#2D5A27] border border-[#D0E8D0] shadow-sm">
            <X size={22} />
          </button>
        </div>
        ...
        <nav className="flex-1 space-y-2 overflow-y-auto pr-1">
          {menuItems.map(item => {
            ...
            return (
              <button key={item.id} onClick={() => handleNavigate(item.id as ViewState)} className={cn(
                'w-full flex items-center gap-3 px-4 py-3 rounded-2xl font-bold transition-all duration-200 text-left',
                isActive ? 'bg-[#D0E8D0] text-[#2D5A27]' : 'text-gray-500 hover:bg-[#E3F0E3] hover:text-[#2D5A27]',
              )}>
